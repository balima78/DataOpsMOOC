...
O pacote [{renv}](https://rstudio.github.io/renv/articles/renv.html) permite criar bibliotecas específicas por projeto (uma biblioteca é um conjunto de pacotes R instalados no nosso computador), registando, num ficheiro `renv.lock`, as versões exatas dos pacotes que foram usados. Este ficheiro vai possibilitar o restauro do ambiente usado noutro computador ou no futuro, isolando o projeto de outras bibliotecas instaladas. Assim conseguimos manter a consistência do ambiente de desenvolvimento ao longo do tempo, em especial permite que possamos trabalhar com versões diferentes de pacotes do R.

Para economizar tempo, numa nova biblioteca criada com o [{renv}](https://rstudio.github.io/renv/articles/renv.html)}, os pacotes necessários são copiados da biblioteca principal do R (caso já estejam instalados na biblioteca padrão), em vez de serem baixados e reinstalados.


## `{renv}`

![](images/renv.svg){width=20%}
